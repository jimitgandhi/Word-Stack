...
public class MainActivity extends AppCompatActivity {
    ...
    public boolean onStartGame(View view) {
        ...
        while(j<word1.length() && k<word2.length()){
            ...
            Log.e("i",i+"");
            if(i==0) {
                scrambledWord+=word1.charAt(j);
                j++;
            }
            else {
                scrambledWord+=word2.charAt(k);
                k++;
            }
        }
        if(j<word1.length()) {
            scrambledWord+=word1.substring(j);
            //j+=word1.substring(j).length();
        }
        if(k<word2.length()){
            scrambledWord+=word2.substring(k);
            //k+=word1.substring(k).length();
        }
